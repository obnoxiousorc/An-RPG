# Grid tile format: {"coords": (0,0), 1: grass, 2: tree, 3: None, "walkable": False, "portal": False}
...
#                                                                                    ^ Whether or not this tile takes you to a new screen
# Note: "grass" and "tree" should be links to the actual image object. In the saved map files, however, they are merely image names.
...
class TileGrid(MapData):
    ...
    def setPortal(self,value,loc,*args):
        self.grid[loc]["portal"] = value
        for loc in args:
            self.grid[loc]["portal"] = value

    def isPortal(self,loc):
        if self.grid[loc]["portal"]:
            return True

    def getPortalMasks(self):
        listOfPortals = []
        for portal in self.portals:
            mask = pygame.mask.from_surface(self.grid[portal][2])
            listOfPortals.append(mask)
